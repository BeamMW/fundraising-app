...
const EpochStatsSection: React.FC<SeedListProps> = ({ className, state }) => {
  ...
  useEffect(() => {
    if (userViewData.current_votes !== undefined) {
      const count = userViewData.current_votes.reduce((total, item) => (item < 255 ? total + 1 : total), 0);
      setVotes(count);
    }
  }, [userViewData]);
  ...
  useEffect(() => {
    ...
    const dateFromString = `${`0${currentTime.getDate()}`.slice(-2)}.${`0${currentTime.getMonth() + 1}`.slice(
      -2,
    )}.${currentTime.getFullYear()}`;
    timestamp += appParams.epoch_dh * 60000;

    const currentPlusOne = new Date(timestamp);
    const dateToString = `${`0${currentPlusOne.getDate()}`.slice(-2)}.${`0${currentPlusOne.getMonth() + 1}`.slice(
      -2,
    )}.${currentPlusOne.getFullYear()}`;
    setNextEpochStartDate(`${dateFromString} - ${dateToString}`);
  }, [blocksLeft, appParams.epoch_dh, systemState.current_state_timestamp]);
  ...
  return (
    <StyledStats className={className}>
      <div className="stats-title-class">
        ...
        {state !== 'stake' && <ExpiresTimer appParams={appParams} systemState={systemState} cHeight={cHeight} />}
      </div>
      <StyledSection>
        <LeftStats>
          <StyledTotalLocked>
        ...
        <MiddleStats isStake={state === 'stake'}>
          <Button pallete="purple" variant="link" onClick={handleDeposit} icon={IconDeposit}>
            deposit
          </Button>
          <Button pallete="blue" className={WithdrawClass} onClick={handleWithdraw} variant="link" icon={IconWithdraw}>
            withdraw
          </Button>
        </MiddleStats>
        {state !== 'stake' ? (
          <Button className={ButtonLinkClass} onClick={handleStakedInfo} pallete="green" variant="link">
            Show staked info
          </Button>
        ) : null}
      </StyledSection>
        ...
        <>
          ...
          <StyledSection>
              ...
              {currentProposals.items.length > 0 && (
                <>
                  ...
                  <div className="progress">
                    <ProgressBar active percent={(votes / currentProposals.items.length) * 100} />
                    <span className="progress-percentage">
                      {parseInt(`${(votes / currentProposals.items.length) * 100}`, 10)}
                      % (
                      {votes}
                      {' '}
                      of
                      {currentProposals.items.length}
                      )
                    </span>
                  </div>
                </>
            ...
            <MiddleStats isStake={false}>
              <div className="next-epoch-title">
                NEXT EPOCH #
            ...
            <Button
              className={ButtonBottomLinkClass}
              onClick={() => navigate(ROUTES.MAIN.FUTURE_EPOCHS)}
              pallete="green"
              variant="link"
            >
              Show future proposals
            </Button>
          </StyledSection>
        </>
      )}
