...
import {
  IconSearchCancel, IconSearch, IconNoProposals, IconVotedYes, IconVotedNo,
...
import { fromGroths, numFormatter, getProposalId } from '@core/appUtils';
...
const ProposalsList: React.FC<ListProps> = ({
  data, extendedData = [], title, type, filter = null,
}) => {
  ...
  useEffect(() => {
    setFilterItems(
      appParams.next.proposals > 0
        ? [appParams.current.iEpoch + 1, appParams.current.iEpoch, ...prevEpoches]
        : [appParams.current.iEpoch, ...prevEpoches],
    );
  ...
  }, [appParams, extendedData, prevEpoches, type]);
  ...
  }, [prevEpoches, appParams]);

  useEffect(() => {
    if (filter) {
      setActiveFilter(filter);
      const filteredExt = extendedEpochs.filter((item) => item === filter);
      setExtendedFilteredEpochs(filteredExt);
    }
  }, [filter, extendedEpochs]);

  useEffect(() => {
    if (searchValue.length > 0 && type === PROPOSALS.PREV) {
      const fitleredData = extendedData
        .filter((item) => {
          if (item.data.title.includes(searchValue) || `${getProposalId(item.id)}`.includes(searchValue)) {
            return item;
          }
          return null;
        })
  ...
  }, [data, selectorActiveItem, searchValue, type, selectorData]);

  useEffect(() => {
    setExtendedEpochs(prevEpoches);

    if (filter) {
      const filteredExt = prevEpoches.filter((item) => item === filter);
      setExtendedFilteredEpochs(filteredExt);
    } else {
  ...
  return (
    <div>
        ...
        {type !== PROPOSALS.FUTURE ? (
          <div className="selector-class">
            {selectorData.map((item, index) => (
              <SelectorItem
                key={index}
                active={selectorActiveItem.id === item.id}
                data-index={index}
      ...
      {items.length > 0 ? (
          ...
          {items.map(
            ...
            <ListItem data-index={index} key={index} onClick={() => handleListItemClick(item.id, index)}>
              <StyledItemHeader className={item.voted !== undefined && item.voted < 255 ? 'voted' : ''}>
                ...
                {type !== PROPOSALS.FUTURE && (item.voted === undefined || item.voted === 255) ? (
              ...
              {type !== PROPOSALS.FUTURE && item.voted !== undefined && item.voted < 255 ? (
                <StyledItemContent>
                  <span className="voted-yes">
                    ...
                    <StyledVotedValue>
                      {numFormatter(fromGroths(item.stats.result.variants[1]))}
                      {' '}
                      BEAMX
                    </StyledVotedValue>
                  </span>
                  <span className="voted-no">
                    ...
                    <StyledVotedValue>
                      {numFormatter(fromGroths(item.stats.result.variants[0]))}
                      {' '}
                      BEAMX
                    </StyledVotedValue>
                  </span>
        ...
        isLoaded
        && (type !== PROPOSALS.PREV || (type === PROPOSALS.PREV && selectorActiveItem.id !== selectorData[0].id)) && (
                  ...
                  && item.epoch === extEpoch && (
                    <ListItem data-index={index} key={index} onClick={() => handleListItemClick(item.id, index)}>
                      ...
                      <StyledItemContent>
                        ...
                        <span className="voted-icon">
                          {(() => {
                            if (item.prevVoted && item.prevVoted.value !== 255) {
                              if (item.prevVoted.value === 1) {
                                return <IconVotedYes />;
                              }
                              return <IconVotedNo />;
                            }
