...
import {
  ROUTES, CID, PROPOSALS, BEAMX_TVL,
...
export function* loadParamsSaga(action: ReturnType<typeof actions.loadAppParams.request>): Generator {
  try {
    ...
    if (!isLoaded) {
      ...
      const parsedCounter = voteCounter && voteCounter.length > 0 ? parseInt(voteCounter, 10) : 0;
      yield put(
        actions.setLocalVoteCounter(parsedCounter > userView.voteCounter ? parsedCounter : userView.voteCounter),
      );
    }
...
export function* loadProposalsSaga(): Generator {
  try {
    ...
    proposalsData.future = nextEpochProps > 0 ? initProposals.splice(initProposals.length - nextEpochProps, nextEpochProps).reverse() : [];
    ...
    const prevEpochVotes = {};
    prevVotes.reduce((accumulator, item) => {
      item.votes.forEach((value, index) => {
        accumulator[item.id1 + index] = { value, stake: item.stake };
      });
      return accumulator;
    }, prevEpochVotes);
    ...
    for (const key of Object.keys(proposalsData)) {
      const proposals = proposalsData[key];
      // eslint-disable-next-line
      for (let item of proposals) {
        item = item as any;
        if (key !== PROPOSALS.PREV) {
          const proposalRes = yield call(LoadProposalData, item.id) as unknown as ProposalStats;
        ...
        if (key === PROPOSALS.CURRENT) {
          ...
          if (userViewData.current_votes !== undefined && userViewData.current_votes.length > 0) {
            item.voted = state.main.userView.current_votes[proposals.length - 1 - proposals.indexOf(item)];
          }
        }
    ...
    for (let i = 0; i < prevProposalsList.length; i += 1) {
        ...
        if (prevProposal.data.quorum !== undefined) {
          if (prevProposal.data.quorum.type === 'beamx') {
            yield put(
              actions.setIsPassed({
                propId: i,
                isPassed: stats.result.variants[1] > toGroths(prevProposal.data.quorum.value),
              }),
            );
            ...
            yield put(actions.setIsPassed({ propId: i, isPassed }));
          }
        } else {
...
export function* loadContractInfoSaga(): Generator {
  try {
    ...
    const state = (yield select()) as { main: EpochesStateType; shared: SharedStateType };
    const epochEndsHeight = state.main.appParams.epoch_dh * state.main.appParams.current.iEpoch + contract.version_history[0].Height;
