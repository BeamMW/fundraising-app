...
const EpochesBase: React.FC = () => {
  ...
  return (
    <>
      <Window>
        <EpochStatsSection state="progress" className={StatsSectionClass} />
        <ProposalsList title="Proposals" type={PROPOSALS.CURRENT} data={currentProposals.items} />
        {prevProposals.items.length > 0 ? (
          <Button variant="ghost" icon={IconOldEpoches} onClick={handleOldEpochs} className={OldButtonClass}>
            show old epochs
          </Button>
        ) : null}
      </Window>
