...
const NewProposalPopup: React.FC<NewProposalPopupProps> = ({ visible, onCancel }) => {
  ...
  const validate = async (formValues: NewProposalFormData) => {
    ...
    if (!REG_AMOUNT.test(quorum_limit) && quorum_limit !== '0' && quorum_limit !== '') {
      ...
      const quorumValue = parseInt(quorum_limit, 10);
      if (quorumValue > 0) {
        if ((!activeToggle && quorumValue > BEAMX_TVL) || (activeToggle && (quorumValue > 100 || quorumValue < 0))) {
          errorsValidation.quorum_limit = 'Incorrect amount.';
        }
      }
    }
  ...
  const submitHandle = (values) => {
    ...
    const qLimit = parseInt(values.quorum_limit, 10);
    if (qLimit > 0) {
      newProposal.quorum = {
        value: qLimit,
  ...
  const isQuorumLimitValid = () => {
    ...
    const quorumValue = parseInt(values.quorum_limit, 10);
    if (quorumValue > 0) {
      if ((!activeToggle && quorumValue > BEAMX_TVL) || (activeToggle && (quorumValue > 100 || quorumValue < 0))) {
        return false;
      }
  ...
  const isTitleValid = () => !errors.voting_title;
  ...
  const isDescriptionValid = () => !errors.voting_descr;
  ...
  const isForumLinkValid = () => !errors.forum_link;
  ...
  const isRefLinkValid = () => !errors.ref_link;
  ...
  return (
    <Popup
      ...
      cancelButton={(
        <Button
          className={NewProposalButtonsClass}
          variant="ghost"
          icon={IconCancel}
          onClick={() => {
            onCancel();
