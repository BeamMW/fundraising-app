...
const WithdrawPopup: React.FC<WithdrawPopupProps> = ({ visible, onCancel }) => {
  ...
  useEffect(() => {
    if (systemState && appParams) {
      let timestamp = systemState.current_state_timestamp * 1000 - (appParams.epoch_dh - blocksLeft) * 60000;
      const currentTime = new Date(timestamp);
      const dateFromString = `${currentTime.getDate()} ${currentTime.toLocaleString('en-US', { month: 'short' })}`;
      timestamp += appParams.epoch_dh * 60000;
      const currentPlusOne = new Date(timestamp);
      const dateToString = `${currentPlusOne.getDate()} ${currentPlusOne.toLocaleString('en-US', { month: 'short' })}`;
      setNextEpochStartDate(
        `${dateFromString} - ${dateToString}, ${currentPlusOne.getFullYear().toString().substr(-2)}`,
      );
    }
  }, [systemState, blocksLeft, appParams, setNextEpochStartDate]);

  const validate = async (formValues: WithdrawFormData) => {
    const errorsValidation: any = {};
    const { withdraw_amount } = formValues;

    const totalStake = fromGroths(userViewData.stake_active + userViewData.stake_passive);
    if (Number(withdraw_amount) > totalStake) {
      errorsValidation.withdraw_amount = `Insufficient funds to complete the transaction. Maximum amount is
       ${totalStake} BEAMX.`;
    }
  ...
  const isWithdrawAmountValid = () => !errors.withdraw_amount;
  ...
  return (
    ...
    >
      ...
      <InfoContainer>
        <div>
          Withdrawing will decrease your voting power in current epoch #
