...
const VotingBar: React.FC<ProgressBarProps> = ({
  active, percent, value, voteType, quorum, qType,
}) => {
  ...
  return (
    <ContainerStyled ref={contRef} voteType={voteType} valueOffset={valueOffset}>
      ...
      {value > 0 && (
        <div ref={valRef} className="value">
          {numFormatter(fromGroths(value))}
          {' '}
          BEAMX
        </div>
      )}
          ...
          content={
            qType === 'beamx' ? (
              <StyledPopover>
                {quorum}
                {' '}
                BEAMX required for approval
              </StyledPopover>
            ) : (
              <StyledPopover>
                {quorum}
                % YES votes required for approval
              </StyledPopover>
            )
          }
        >
          <QuorumLine
            margin={qType === 'percent' ? quorum : (quorum / fromGroths(totalsView.stake_active)) * 100}
            percent={percent}
