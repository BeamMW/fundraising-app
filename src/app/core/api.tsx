...
export function LoadTotals<T = any>(): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=manager,action=view_totals,cid=${CID}`, (error, result) => {
      resolve(result.res);
    });
  });
}
...
export function LoadProposals<T = any>(): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=manager,action=view_proposals,cid=${CID}`, (error, result) => {
      resolve(result.res);
    });
  });
}
...
export function LoadProposalData<T = any>(id): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=manager,action=view_proposal,id=${id},cid=${CID}`, (error, result) => {
      resolve(result);
    });
  });
}
...
export function LoadManagerView<T = any>(): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=manager,action=view,cid=${CID}`, (error, result) => {
      resolve(result);
    });
  });
}
...
export function LoadModeratorsView<T = any>(): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=manager,action=view_moderators,cid=${CID}`, (error, result) => {
      resolve(result.res);
    });
  });
}
...
export function LoadPublicKey<T = any>(): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=user,action=my_key,cid=${CID}`, (error, result) => {
      resolve(result.key);
    });
  });
}
...
export function VoteProposal<T = any>(votes: number[], id: number, vote: number, counter: number): Promise<T> {
  return new Promise((resolve) => {
    ...
    Utils.invokeContract(req, (error, result, full) => {
...
export function LoadUserView<T = any>(): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=user,action=view,cid=${CID}`, (error, result) => {
      resolve(result.res);
    });
  });
}
...
export function LoadVotes<T = any>(): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=user,action=view_votes,cid=${CID}`, (error, result) => {
      resolve(result.res);
    });
  });
}
...
export function UserDeposit<T = any>(amount): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=user,action=move_funds,amount=${amount},bLock=1,cid=${CID}`, (error, result, full) => {
...
export function UserWithdraw<T = any>(amount): Promise<T> {
  return new Promise((resolve) => {
    Utils.invokeContract(`role=user,action=move_funds,amount=${amount},bLock=0,cid=${CID}`, (error, result, full) => {
      onMakeTx(error, result, full);
      resolve(result);
    });
  });
